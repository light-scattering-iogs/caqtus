...
from sqlalchemy.orm import Mapped, mapped_column, Session, relationship, foreign, remote
...
class Path(Base):
    ...
    descendants: Mapped[list[Path]] = relationship(
        viewonly=True,
        order_by=path,
        primaryjoin=remote(foreign(path)).like(path.concat(f"{_PATH_SEPARATOR}%")),
    )
