...
class SQLSequenceHierarchy(SequenceHierarchy):
    ...
    def create_path(
        ...
        for path_to_create in paths_to_create:
            Path.create(str(path_to_create), session)
    ...
    def _query_path_model(self, path: SequencePath) -> Result[Path, PathNotFoundError]:
        stmt = select(Path).where(Path.path == str(path))
    ...
    def _query_path_models(self, paths: Iterable[SequencePath]) -> list[Path]:
