...
from caqtus.types.data import DataSchema
...
class DeviceExtension(Generic[DeviceConfigT]):
        ...
        compiler_type: A function that creates a compiler used to compile the parameters
            ...
            Warning:
                ...
                - :func:`print` statements in the compiler will not be displayed in the
                  console.
        data_schema: A function that returns the data schema of the device.

            This function is called with the name of the device and the context of the
            sequence as arguments.
            It must return the type of each data generated by the device for each shot
            of the sequence.
            This constrains devices to always return the same data type for each shot.
            If the device does not generate any data or is not used during the sequence,
            it should return an empty schema.
            If during any shot, the device generates data that does not match the
            schema, it will crash the sequence execution.
    """
    ...
    data_schema: Callable[[DeviceName, SequenceContext], DataSchema] = attrs.field()
