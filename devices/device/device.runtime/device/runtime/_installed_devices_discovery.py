...
def discover_installed_devices() -> dict[str, type[Device]]:
    ...
    `experiment_control.devices`.

    Returns:
        A dictionary matching the entry point name to the class indicated by the object reference in the entry point.
        The class referenced must be a subclass of <Device>. It can be an implicit subclass, as long as it has the
        required methods.
    ...
    for entry_point in discovered_entry_points:
        if entry_point.name in result:
            raise ValueError(
                f"Device entry point `{entry_point.name}` cannot be used more than once."
            )
        loaded = discovered_entry_points[entry_point.name].load()
        if not issubclass(loaded, Device):
            raise TypeError(
                f"Entry point {entry_point.name} doesn't reference a <Device> subclass, instead got {loaded}"
            )
