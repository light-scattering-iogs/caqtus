...
from sequencer.instructions import RepeatOld, SequencerPattern, ChannelLabel
...
def test_split():
    ...
    instruction = RepeatOld(
        SequencerPattern({ChannelLabel(1): ChannelPattern((1,))}), 20
    )
    ...
    assert right == RepeatOld(
        SequencerPattern({ChannelLabel(1): ChannelPattern((1,))}), 10
    )
